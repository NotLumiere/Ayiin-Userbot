...
from pyAyiin import ayiin, startTime
from pyAyiin.database.botBlacklist import checkIsBlacklist
from pyAyiin.database.botStart import (
    addStarterToDb,
    getAllStarters,
    getStarterDetails,
)
from pyAyiin.database.variable import getVar
from pyAyiin.decorator import asstCmd, callBack
...
async def check_bot_started_users(user, event):
    if user.id == ayiin.me.id:
        return
    check = getStarterDetails(user.id)
    if check is None:
        start_date = str(datetime.now().strftime("%B %d, %Y"))
        notification = f"🔮 **#BOT_START**\n**First Name:** {ayiin.mentionuser(user.first_name , user.id)} \
                \n**User ID: **`{user.id}`\
    ...
    try:
        ...
        ayiin.log.error(str(e))
    if ayiin.BOTLOG_CHATID:
        await event.client.send_message(ayiin.BOTLOG_CHATID, notification)


@callBack(data=re.compile(b"pmclose"))
async def pmclose(event):
    if event.query.user_id == ayiin.me.id:
        await event.delete()
...
async def alvlogo(event):
    ...
    async with event.client.conversation(pru) as conv:
        ...
        await ayiin.setVarValue(vars=var, value=url)
        await conv.send_message(
...
async def alvmoji(event):
    ...
    async with event.client.conversation(pru) as conv:
        ...
        try:
            ...
            return await conv.send_message(
                f"Membatalkan Proses Settings VAR {var}\n\n**Karena Tidak Ada Respon Dari User**",
            )
        themssg = response.message.message
        if themssg == "/cancel":
            return await conv.send_message(
        ...
        await ayiin.setVarValue(var, themssg)
        await conv.send_message(
...
async def inpics(event):
    ...
    async with event.client.conversation(pru) as conv:
        ...
        await ayiin.setVarValue(var, url)
        await conv.send_message(
            f"**{var} Berhasil di Ganti Tod**\n\nSabar Ya Babi Ini Sedang MeRestart Heroku untuk Menerapkan Perubahan.",
            buttons=get_back_button("inlinemenu"),
        )
...
@asstCmd(pattern="^/start?([\\s]+)?$", func=lambda e: e.is_private)
async def bot_start(event):
    chat = await event.get_chat()
    user = await event.client.get_me()
    if checkIsBlacklist(chat.id):
        return
    owner = ayiin.me
    ...
    if chat.id != owner.id:
        customstrmsg = getVar("START_TEXT") or None
        ...
        buttons = [
            (
                Button.url("ɢʀᴏᴜᴘ", f"https://t.me/{ayiin._gc}"),
                Button.url("ᴄʜᴀɴɴᴇʟ", f"https://t.me/{ayiin._ch}"),
            )
        ]
    else:
...
@asstCmd(pattern="^/ping$")
async def _(event):
    start = datetime.now()
    end = datetime.now()
    duration = (end - start).microseconds / 1000
    await ayiin.bot.send_message(event.chat_id, "🏓**Pong!**\n`%sms`" % duration)
